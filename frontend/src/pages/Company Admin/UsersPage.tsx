  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Users</h2>
          <p className="text-sm text-slate-500 mt-1">Everyone with access to your company's account.</p>
        </div>
        <button
          onClick={() => setShowAddForm(true)}
          className="flex items-center gap-2 rounded-md bg-slate-900 text-white text-sm font-medium px-4 py-2 hover:bg-slate-800 transition"
        >
          <UserPlus size={16} />
          Add user
        </button>
      ...
      {users === null && !error && (
        <div className="text-sm text-slate-400 py-8 text-center">Loading users…</div>
      )}
